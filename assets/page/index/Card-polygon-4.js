...
var Card = (function(window, undefined) {
  ...
  function Card(id, el) {
    ...
    // Get elements.
    this._container = $cash(this._el).find(SELECTORS.container)[0];
    this._clip = $cash(this._el).find(SELECTORS.clip)[0];
    this._content = $cash(this._el).find(SELECTORS.content)[0];
    ...
    start.forEach(function(point, i) {
      ...
      var tween = TweenLite.to(point, 1.5, end[i]);
        ...
        if (points.length === end.length) {
          $cash(this._clip).attr('points', points.join(' '));
          // Reset.
          points = [];
        };
  ...
  Card.prototype._floatContainer = function(callback) {
    ...
    $cash(document.body).addClass(CLASSES.bodyHidden);
    ...
    TL.to([this._container, track], 2, {
      ...
      onComplete: function() {
        $cash(this._container).addClass('card__container--fix-image');
      }.bind(this)
    });
  ...
  Card.prototype.closeCard = function() {
    ...
    TweenLite.to(this._container, 0.4, {
      ...
      onComplete: function() {
        $cash(this._container).css('overflow', 'hidden');
      }.bind(this),
      ease: Power2.easeOut
    });
    ...
    this._TL.eventCallback('onReverseComplete', function() {
      ...
      $cash(document.body).removeClass(CLASSES.bodyHidden);
